...
def save_model(args, step, model_without_ddp, optimizer, loss_scaler, fname=None, best_so_far=None):
    output_dir = Path(args.output_dir)
    if fname is None: fname = str(step)
...
def format_ckpt_for_compile(model, ckpt):
    # Check if the state dict keys contain "_orig_mod."
    has_orig_mod = any(k.startswith("_orig_mod.") for k in ckpt['model'].keys())

    # Fix keys only if the model was compiled but args.compile is False
    if has_orig_mod and not is_model_compiled(model):
...
def load_model(args, model_without_ddp, optimizer, loss_scaler):
    ...
    if args.resume is not None:
        ...
        # Align name of the model's keys with the checkpoint's keys if needed for compile
        format_ckpt_for_compile(model_without_ddp, checkpoint)
...
def get_parameter_groups(model, weight_decay, layer_decay=1.0, skip_list=(), no_lr_scale_list=[]):
    ...
    for name, param in model.named_parameters():
        ...
        if len(param.shape) == 1 or name.endswith(".bias") or name in skip_list or any(name.startswith(prefix) for prefix in skip_list):
            group_name = "no_decay"
            this_weight_decay = 0.
        else:
            group_name = "decay"
...
def adjust_lr(optimizer, step, args):
    ...
    if step < args.warmup_steps:
        ...
        lr = args.min_lr + (args.lr - args.min_lr) * 0.5 * \
            (1. + math.cos(math.pi * (step - args.warmup_steps) / (args.steps - args.warmup_steps)))
...
def breakdown_grads(model, metric_logger):
    enc_norm = torch.nn.utils.clip_grad_norm_(model.module.enc_blocks.parameters(), float('inf'))
    dec_norm = torch.nn.utils.clip_grad_norm_(model.module.dec_blocks.parameters(), float('inf'))
    patch_norm = torch.nn.utils.clip_grad_norm_(model.module.patch_embed.parameters(), float('inf'))
    prediction_head_norm = torch.nn.utils.clip_grad_norm_(model.module.prediction_head.parameters(), float('inf'))
    if isinstance(model.module.mask_token, torch.nn.Parameter):
        mask_token_norm = torch.nn.utils.clip_grad_norm_([model.module.mask_token], float('inf')) 
    else:
        mask_token_norm = torch.nn.utils.clip_grad_norm_(model.module.mask_token.parameters(), float('inf'))

    if hasattr(model.module, "pos_emb_E_encoder") and hasattr(model.module, "pos_emb_E_decoder"):
        pos_enc_norm = torch.nn.utils.clip_grad_norm_(
            [model.module.pos_emb_E_encoder, model.module.pos_emb_E_decoder], float('inf')
        )
    else:
        pos_enc_norm = 0.0 
    grad_norms = {
        "enc_grad": enc_norm,
        "dec_grad": dec_norm,
        "patch_grad": patch_norm,
        "pred_head_grad": prediction_head_norm,
        "mask_token_grad": mask_token_norm,
        "pos_enc_grad": pos_enc_norm,
    }
    metric_logger.update(**grad_norms)
    return 
